...
pub const buffers = struct {
    ...
    pub var B = usb.Buffers{
        .ep0_buffer0 = @ptrFromInt(USB_EP0_BUFFER0),
        .ep0_buffer1 = @ptrFromInt(USB_EP0_BUFFER1),
    ...
    pub fn usb_init_device(device_config: *usb.DeviceConfiguration) void {
        ...
        for (device_config.endpoints) |ep| {
            ...
            if (ep.endpoint_control_index) |epci| {
                ...
                const dpram_offset: u16 = @intCast(buf_base - dpram_base);
...
pub fn next(self: *usb.EPBIter) ?usb.EPB {
    ...
    const epnum: u8 = @intCast(lowbit_index >> 1);
    ...
    const len: usize = @intCast(bc & 0x3ff);
